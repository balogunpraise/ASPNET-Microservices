...
namespace Discount.Grpc.Repositories
{
    public class DiscountRepository : IDiscountRepository
    {
        ...
        public DiscountRepository(IConfiguration config)
        {
            ...
            ConnectionString = _config.GetValue<string>("DatabaseSettings:ConnectionString");
        }
        #region Command
        ...
        public async Task<bool> UpdateDiscount(Coupon coupon)
        {
            ...
            var affected = await connection.ExecuteAsync("UPDATE Coupon SET ProductName=@ProductName, Description=@Description, Amount=@Amount " +
                "WHERE Id = @Id", new { ProductName = coupon.ProductName, Description = coupon.Description, Amount = coupon.Amount });
            if (affected == 0) return false;
            return true;
        }
